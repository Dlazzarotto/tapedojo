...
// Lê o ref do projeto embutido num JWT do Supabase.
export function jwtRef(token) {
  try {
    const seg = (token || "").split(".");
    if (seg.length !== 3) return null;
    const b64 = seg[1].replace(/-/g, "+").replace(/_/g, "/");
...
// Cofre = fonte da verdade dos inscritos; td_profiles enriquece (tier/país).
export function authToProfiles(authUsers, profileRows) {
  const byId = {};
  (profileRows || []).forEach((p) => { if (p && p.user_id) byId[p.user_id] = p; });
  return (authUsers || []).filter((u) => u && u.id).map((u) => {
    const p = byId[u.id] || {};
    return {
      user_id: u.id,
      display_name: p.display_name || (u.user_metadata && u.user_metadata.display_name) || ((u.email || "aluno").split("@")[0]),
      tier: p.tier || null,
...
// Impressão digital da chave em uso — para o rodapé confessar o que está vivo.
export function keyKind(token) {
  const t = token || "";
  if (t.startsWith("sb_secret_")) return "sb_secret (formato novo)";
  if (t.startsWith("sb_publishable_")) return "sb_publishable (PÚBLICA!)";
  const role = jwtRole(t);
  if (role === "service_role") return "legada service_role ✓";
  if (role === "anon") return "legada ANON (pública!)";
  if (role) return "JWT papel '" + role + "'";
  return "desconhecida (" + t.length + " caracteres)";
}
