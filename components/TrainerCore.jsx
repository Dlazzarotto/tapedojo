...
const ALLOWANCE = { free: 1000, base: 1000, plus: 3000, master: 10000 }; // pontos/mês
const REGEN_PER_HOUR = 10;  // Master: regeneração quando os pontos acabam
const REGEN_CAP = 100;      // teto do acúmulo por regeneração
  ...
  pt: {
    ...
    ui: {
      ...
      plans: {
        ...
        master: { name: "Master", desc: ALLOWANCE.master.toLocaleString("pt-BR") + " pontos/mês + regeneração automática · mercado AO VIVO (níveis 4–6)." },
      },
  ...
  en: {
    ...
    ui: {
      ...
      plans: {
        ...
        master: { name: "Master", desc: ALLOWANCE.master.toLocaleString("en-US") + " points/month + auto regeneration · LIVE market (levels 4–6)." },
      },
    ...
    cats: {
      absorcao: {
        ...
        tapeBuy: ["Large lots hitting the bid relentlessly, yet price prints no new low — passive buy liquidity reloads at the same level.", "Sellers have been dumping heavy lots into the bid for minutes and price simply will not give way — every wave meets passive buying waiting at the same level.", "The time and sales shows a string of aggressive sells at the low, yet no new low prints — the buy queue in the book refills as fast as it is consumed.", "Large sell order after large sell order — and the session low won't budge; someone is loading the truck in silence.", "Every burst of aggressive selling wipes the bid from the level… and it reappears reloaded seconds later, always at the same price.", "Obvious selling pressure on the tape, yet the candle won't stretch: the sells vanish into an invisible wall of bids.", "Iceberg on the bid: the tape prints sell after sell and the quote gives back pennies — the seller tires before the buyer.", "Huge volume at the low with price standing still: the auction just met a buyer the size of the market.", "The sells crash into the same level like waves on a seawall — plenty of noise on the tape, not one print at a new low."],
        tapeSell: ["Large lots hitting the ask relentlessly, yet price prints no new high — passive sell liquidity reloads at the same level.", "Buyers have been dumping heavy lots into the ask for minutes and price simply will not advance — every wave meets passive selling waiting at the same level.", "The time and sales shows a string of aggressive buys at the high, yet no new high prints — the sell queue in the book refills as fast as it is consumed.", "Large buy order after large buy order — and the session high stays intact; someone is distributing in silence.", "Every burst of aggressive buying wipes the ask from the level… and it reappears reloaded seconds later, at the same price.", "Obvious buying pressure on the tape, yet the candle won't stretch: the buys vanish into an invisible wall of offers.", "Iceberg on the ask: the tape prints buy after buy and the quote gives back pennies — the buyer tires before the seller.", "Huge volume at the high with price standing still: the auction just met a seller the size of the market.", "The buys crash into the same level like waves on a seawall — plenty of noise on the tape, not one print at a new high."],
      ...
      divergencia: {
        ...
        tapeSell: ["Price prints higher highs, but buyer aggression thins with every push — aggressor lot sizes shrink.", "Each new high prints with fewer aggressive trades than the last — price keeps advancing while buyer participation dries up.", "Price renews the top, yet the tape shows smaller buyer aggression on every leg — the big prints are gone.", "High after high on the chart, but each push prints with fewer buying lots — the rally runs on inertia, not aggression.", "Buyer delta shrinks with every leg up: price still advances, but whoever was pushing no longer shows on the tape.", "New high on the print, half the aggression: the big buyers are gone and retail is stretching the move.", "Each fresh high gets a weaker answer from the tape — fewer trades, smaller lots, pressure evaporating.", "Price pierces resistance, but aggressive buy volume is a fraction of what it was two legs ago.", "A rally running out of fuel: marginal highs, buyer delta shallower on every attempt."],
        tapeBuy: ["Price prints lower lows, but seller aggression thins with every drop — aggressor lot sizes shrink.", "Each new low prints with fewer aggressive trades than the last — price keeps falling while seller participation dries up.", "Price renews the bottom, yet the tape shows smaller seller aggression on every leg — the big prints are gone.", "Low after low on the chart, but each dip prints with fewer selling lots — the decline runs on inertia, not aggression.", "Seller delta shrinks with every leg down: price still gives way, but whoever was pushing no longer shows on the tape.", "New low on the print, half the aggression: the big sellers are gone and retail is stretching the move.", "Each fresh low gets a weaker answer from the tape — fewer trades, smaller lots, pressure evaporating.", "Price pierces support, but aggressive sell volume is a fraction of what it was two legs ago.", "A decline running out of fuel: marginal lows, seller delta shallower on every attempt."],
      ...
      iniciativa: {
        ...
        tapeBuy: ["At the break, the tape accelerates: large lots hitting the ask in sequence, the sell book consumed level after level, no reload.", "The level breaks with large, rapid prints on the ask; sell liquidity vanishes from the screen without being replaced.", "At the range break, tape speed explodes: buyer aggression in bursts, the sell book evaporating level by level.", "The range dies with a snap: a string of full-size lots into the ask, every new offer swallowed on arrival.", "A breakout with an institutional signature — big prints, quickening pace, and the offer side backing away without reloading.", "The buying arrives in short, heavy bursts; the ask can't hold the same price for two seconds.", "After the break, the tape won't let price return: every fresh offer is consumed before a queue can form.", "Speed and size together: the tape fires upward with above-average lots and zero reload from sellers.", "The level gives way and the market changes hands — continuous buying aggression, no pause for the seller to breathe."],
        tapeSell: ["At the break, the tape accelerates: large lots hitting the bid in sequence, the buy book consumed level after level, no reload.", "The level breaks with large, rapid prints on the bid; buy liquidity vanishes from the screen without being replaced.", "At the range break, tape speed explodes: seller aggression in bursts, the buy book evaporating level by level.", "The range dies with a snap: a string of full-size lots into the bid, every new bid swallowed on arrival.", "A breakdown with an institutional signature — big prints, quickening pace, and the bid side backing away without reloading.", "The selling arrives in short, heavy bursts; the bid can't hold the same price for two seconds.", "After the break, the tape won't let price return: every fresh bid is consumed before a queue can form.", "Speed and size together: the tape plunges with above-average lots and zero reload from buyers.", "The level gives way and the market changes hands — continuous selling aggression, no pause for the buyer to breathe."],
      ...
      exaustao: {
        ...
        tapeSell: ["A burst of trades — the largest volume of the session — buyers aggressing in panic, yet price stretches, gives it all back and closes near the open.", "A historic burst of trades for the session: cascading aggressive buying, yet the candle gives the advance back and closes where it started.", "The tape boils at the top — everyone buying at once — and still price cannot hold the stretch, sliding back to the open.", "Classic climax: the tape prints the day's biggest buying burst and the candle closes far off the high, giving nearly all of it back.", "An avalanche of buying at the high — and the giant upper wick tells the rest: the whole spike got sold.", "All of the day's buying euphoria fit inside a candle that barely moved: record effort, zero result.", "Volume explodes at the high, price stretches… and returns within the same bar — the last buyer just stepped in.", "Final burst: trades at a session record, a marginal high rejected, and a close right back where it started.", "The tape screams buy but the candle whispers sell: a stretched range handed back, closing in the lower third."],
        tapeBuy: ["A burst of trades — the largest volume of the session — sellers aggressing in panic, yet price stretches down, gives it back and closes near the open.", "A historic burst of trades for the session: cascading aggressive selling, yet the candle gives the drop back and closes where it started.", "The tape boils at the low — everyone selling at once — and still price cannot hold the stretch, sliding back to the open.", "Classic capitulation: the tape prints the day's biggest selling burst and the candle closes far off the low, giving nearly all of it back.", "An avalanche of selling at the low — and the giant lower wick tells the rest: the whole plunge got bought.", "All of the day's selling panic fit inside a candle that barely moved: record effort, zero result.", "Volume explodes at the low, price stretches… and returns within the same bar — the last seller just left.", "Final burst: trades at a session record, a marginal low rejected, and a close right back where it started.", "The tape screams sell but the candle whispers buy: a stretched range handed back, closing in the upper third."],
      ...
      pullback: {
        ...
        tapeBuy: ["On the dip, the tape empties: small, spaced trades, no meaningful lot hitting the bid; the book below stays intact.", "The dip trickles in: tiny prints, no institutional lot selling; the buying structure below remains in place.", "During the correction the tape nearly stalls — minimal volume, irrelevant aggression — while buy liquidity stays positioned.", "The correction drifts sideways, unhurried and lot-less: no size selling, just trickled profit-taking.", "The dip feels like an intermission: slow tape, small prints, and the structured demand still parked just below.", "Three down candles and not one institutional footprint on the tape — dry volume, irrelevant delta.", "Price breathes, the tape naps: the pullback happens in a vacuum, with no selling aggression worth noting.", "A skinny correction after a fat leg: the lots that drove the market up are nowhere on the sell side.", "The countermove comes without conviction — sparse trades, tiny size, bid stack intact."],
        tapeSell: ["On the bounce, the tape empties: small, spaced trades, no meaningful lot hitting the ask; the book above stays intact.", "The bounce trickles in: tiny prints, no institutional lot buying; the selling structure above remains in place.", "During the correction the tape nearly stalls — minimal volume, irrelevant aggression — while sell liquidity stays positioned.", "The bounce drifts sideways, unhurried and lot-less: no size buying, just trickled short-covering.", "The bounce feels like an intermission: slow tape, small prints, and the structured supply still parked just above.", "Three up candles and not one institutional footprint on the tape — dry volume, irrelevant delta.", "Price breathes, the tape naps: the bounce happens in a vacuum, with no buying aggression worth noting.", "A skinny bounce after a fat leg: the lots that drove the market down are nowhere on the buy side.", "The countermove comes without conviction — sparse trades, tiny size, offer stack intact."],
      ...
      mistos: {
        ...
        tape: ["The tape alternates: strong aggression one candle, absence the next; delta and price tell different stories on consecutive legs.", "One candle brings heavy aggression, the next comes empty; delta points one way and the close goes the other — nothing confirms.", "The tape builds no narrative: isolated large prints with no follow-through, price and delta contradicting each other leg after leg.", "The tape changes its mind every candle: buying aggression now, selling next, and price validates neither.", "Heavy volume here, silence there; positive delta on one print, negative on the next — the auction has no owner.", "Nothing adds up: a candle rises on seller delta, the next falls on buyer delta, and no level gets defended twice.", "Signals in short circuit — every piece of evidence points one way and the next one contradicts it.", "The market prints noise with volume: moves that start strong and die on the following candle.", "A deliberately blurry read: isolated bursts of aggression, no sequence, no defense, no story."],
  ...
  es: {
    ...
    ui: {
      ...
      plans: {
        ...
        master: { name: "Master", desc: ALLOWANCE.master.toLocaleString("es") + " puntos/mes + regeneración automática · mercado EN VIVO (niveles 4–6)." },
      },
    ...
    cats: {
      absorcao: {
        ...
        tapeBuy: ["Lotes grandes golpeando el bid sin parar, pero el precio no hace nuevo mínimo — la liquidez pasiva de compra recarga en el mismo nivel.", "Los vendedores descargan lotes pesados en el bid durante minutos y el precio simplemente no cede — cada oleada encuentra compra pasiva esperando en el mismo nivel.", "El times & trades muestra ventas agresivas en serie en el mínimo, pero no imprime nuevo mínimo — la cola de compra del book se recompone apenas se consume.", "Orden grande de venta tras orden grande de venta — y el mínimo del día sigue intacto; alguien llena el camión en silencio.", "Cada ráfaga de venta agresiva borra el bid del nivel… y reaparece recargado segundos después, siempre al mismo precio.", "Presión vendedora evidente en el tape, pero la vela no estira: las ventas desaparecen dentro de una pared invisible de compras.", "Iceberg en el bid: el tape imprime venta tras venta y la cotización devuelve centavos — el vendedor se cansa antes que el comprador.", "Volumen gigante en el mínimo con el precio quieto: la subasta encontró un comprador del tamaño del mercado.", "Las ventas golpean el mismo nivel como olas contra el rompeolas — mucho ruido en el tape, ni un print de nuevo mínimo."],
        tapeSell: ["Lotes grandes golpeando el ask sin parar, pero el precio no hace nuevo máximo — la liquidez pasiva de venta recarga en el mismo nivel.", "Los compradores descargan lotes pesados en el ask durante minutos y el precio simplemente no avanza — cada oleada encuentra venta pasiva esperando en el mismo nivel.", "El times & trades muestra compras agresivas en serie en el máximo, pero no imprime nuevo máximo — la cola de venta del book se recompone apenas se consume.", "Orden grande de compra tras orden grande de compra — y el máximo del día sigue intacto; alguien distribuye en silencio.", "Cada ráfaga de compra agresiva borra el ask del nivel… y reaparece recargado segundos después, al mismo precio.", "Presión compradora evidente en el tape, pero la vela no estira: las compras desaparecen dentro de una pared invisible de ventas.", "Iceberg en el ask: el tape imprime compra tras compra y la cotización devuelve centavos — el comprador se cansa antes que el vendedor.", "Volumen gigante en el máximo con el precio quieto: la subasta encontró un vendedor del tamaño del mercado.", "Las compras golpean el mismo nivel como olas contra el rompeolas — mucho ruido en el tape, ni un print de nuevo máximo."],
      ...
      divergencia: {
        ...
        tapeSell: ["El precio imprime máximos más altos, pero la agresión compradora se adelgaza en cada avance — los lotes agresores se encogen.", "Cada nuevo máximo sale con menos negocios agresores que el anterior — el precio sigue avanzando mientras la participación compradora se seca.", "El precio renueva el techo, pero el tape muestra agresión compradora cada vez menor en cada pierna — los prints grandes desaparecieron.", "Máximo tras máximo en el gráfico, pero cada estirón sale con menos lotes compradores — la subida avanza por inercia, no por agresión.", "El delta comprador se encoge en cada tramo alcista: el precio aún avanza, pero quien empujaba ya no aparece en el tape.", "Nuevo máximo en el print, la mitad de la agresión: los compradores grandes se fueron y el minorista estira el movimiento.", "Cada renovación del máximo recibe una respuesta más débil del tape — menos negocios, lotes más pequeños, presión evaporándose.", "El precio perfora la resistencia, pero el volumen agresor de compra es una fracción del de dos tramos atrás.", "Una subida sin combustible: máximos marginales, delta comprador cada vez más raso en cada intento."],
        tapeBuy: ["El precio imprime mínimos más bajos, pero la agresión vendedora se adelgaza en cada caída — los lotes agresores se encogen.", "Cada nuevo mínimo sale con menos negocios agresores que el anterior — el precio sigue cayendo mientras la participación vendedora se seca.", "El precio renueva el piso, pero el tape muestra agresión vendedora cada vez menor en cada pierna — los prints grandes desaparecieron.", "Mínimo tras mínimo en el gráfico, pero cada caída sale con menos lotes vendedores — la baja avanza por inercia, no por agresión.", "El delta vendedor se encoge en cada tramo bajista: el precio aún cede, pero quien empujaba ya no aparece en el tape.", "Nuevo mínimo en el print, la mitad de la agresión: los vendedores grandes se fueron y el minorista estira el movimiento.", "Cada renovación del mínimo recibe una respuesta más débil del tape — menos negocios, lotes más pequeños, presión evaporándose.", "El precio perfora el soporte, pero el volumen agresor de venta es una fracción del de dos tramos atrás.", "Una caída sin combustible: mínimos marginales, delta vendedor cada vez más raso en cada intento."],
      ...
      iniciativa: {
        ...
        tapeBuy: ["En la ruptura, el tape acelera: lotes grandes golpeando el ask en secuencia y el book de venta consumido nivel tras nivel, sin recarga.", "El nivel se rompe con prints grandes y rápidos en el ask; la liquidez de venta desaparece de la pantalla sin reponerse.", "En la ruptura del rango, la velocidad del tape se dispara: agresión compradora en ráfagas, el book de venta evaporándose nivel a nivel.", "El rango muere de golpe: secuencia de lotes llenos contra el ask, y cada oferta nueva es tragada al instante.", "Ruptura con firma institucional — prints grandes, ritmo acelerando y el lado vendedor retrocediendo sin recargar.", "La compra llega en ráfagas cortas y pesadas; el ask no aguanta dos segundos en el mismo precio.", "Tras la ruptura, el tape no deja volver al precio: toda oferta nueva se consume antes de formar fila.", "Velocidad y tamaño juntos: el tape dispara al alza con lotes sobre la media y cero recarga del vendedor.", "El nivel cede y el mercado cambia de dueño — agresión compradora continua, sin pausa para que el vendedor respire."],
        tapeSell: ["En la ruptura, el tape acelera: lotes grandes golpeando el bid en secuencia y el book de compra consumido nivel tras nivel, sin recarga.", "El nivel se rompe con prints grandes y rápidos en el bid; la liquidez de compra desaparece de la pantalla sin reponerse.", "En la ruptura del rango, la velocidad del tape se dispara: agresión vendedora en ráfagas, el book de compra evaporándose nivel a nivel.", "El rango muere de golpe: secuencia de lotes llenos contra el bid, y cada puja nueva es tragada al instante.", "Quiebre con firma institucional — prints grandes, ritmo acelerando y el lado comprador retrocediendo sin recargar.", "La venta llega en ráfagas cortas y pesadas; el bid no aguanta dos segundos en el mismo precio.", "Tras el quiebre, el tape no deja volver al precio: toda puja nueva se consume antes de formar fila.", "Velocidad y tamaño juntos: el tape se desploma con lotes sobre la media y cero recarga del comprador.", "El nivel cede y el mercado cambia de dueño — agresión vendedora continua, sin pausa para que el comprador respire."],
      ...
      exaustao: {
        ...
        tapeSell: ["Explosión de negocios — el mayor volumen de la sesión — con compradores agrediendo en pánico, pero el precio se estira, lo devuelve casi todo y cierra cerca de la apertura.", "Pico histórico de negocios en la sesión: compra agresiva en cascada, pero la vela devuelve el avance y cierra donde empezó.", "El tape hierve en el techo — todos comprando a la vez — y aun así el precio no sostiene el estirón, volviendo a la apertura.", "Clímax clásico: el tape imprime la mayor ráfaga compradora del día y la vela cierra lejos del máximo, devolviendo casi todo.", "Compra en avalancha en el máximo — y la mecha superior gigante cuenta el resto: todo el estirón fue vendido.", "Toda la euforia compradora del día cupo en una vela que apenas se movió: esfuerzo récord, resultado nulo.", "El volumen explota en el máximo, el precio estira… y regresa en la misma vela — el último comprador acaba de entrar.", "Ráfaga final: negocios en récord de la sesión, máximo marginal rechazado y cierre de vuelta al punto de partida.", "El tape grita compra pero la vela susurra venta: rango estirado devuelto y cierre en el tercio inferior."],
        tapeBuy: ["Explosión de negocios — el mayor volumen de la sesión — con vendedores agrediendo en pánico, pero el precio se estira a la baja, lo devuelve y cierra cerca de la apertura.", "Pico histórico de negocios en la sesión: venta agresiva en cascada, pero la vela devuelve la caída y cierra donde empezó.", "El tape hierve en el piso — todos vendiendo a la vez — y aun así el precio no sostiene el estirón, volviendo a la apertura.", "Capitulación clásica: el tape imprime la mayor ráfaga vendedora del día y la vela cierra lejos del mínimo, devolviendo casi todo.", "Venta en avalancha en el mínimo — y la mecha inferior gigante cuenta el resto: toda la caída fue comprada.", "Todo el pánico vendedor del día cupo en una vela que apenas se movió: esfuerzo récord, resultado nulo.", "El volumen explota en el mínimo, el precio estira… y regresa en la misma vela — el último vendedor acaba de salir.", "Ráfaga final: negocios en récord de la sesión, mínimo marginal rechazado y cierre de vuelta al punto de partida.", "El tape grita venta pero la vela susurra compra: rango estirado devuelto y cierre en el tercio superior."],
      ...
      pullback: {
        ...
        tapeBuy: ["En el retroceso, el tape se vacía: negocios pequeños y espaciados, ningún lote relevante golpeando el bid; el book de abajo sigue intacto.", "El retroceso llega a cuentagotas: prints pequeños, ningún lote institucional vendiendo; la estructura compradora de abajo permanece.", "Durante la corrección el tape casi se detiene — volumen mínimo, agresión irrelevante — mientras la liquidez de compra sigue posicionada.", "La corrección baja de lado, sin prisa y sin lotes: nadie grande vendiendo, solo toma de ganancias goteada.", "El retroceso parece un intermedio: tape lento, prints pequeños, y la demanda estructurada sigue montada justo debajo.", "Tres velas de baja y ni una huella institucional en el tape — volumen seco, delta irrelevante.", "El precio respira, el tape dormita: el retroceso ocurre en el vacío, sin agresión vendedora digna de nota.", "Corrección flaca tras un tramo gordo: los lotes que subieron el mercado no aparecen del lado de la venta.", "El movimiento contrario llega sin convicción — negocios dispersos, tamaño diminuto, book comprador intacto."],
        tapeSell: ["En el rebote, el tape se vacía: negocios pequeños y espaciados, ningún lote relevante golpeando el ask; el book de arriba sigue intacto.", "El rebote llega a cuentagotas: prints pequeños, ningún lote institucional comprando; la estructura vendedora de arriba permanece.", "Durante la corrección el tape casi se detiene — volumen mínimo, agresión irrelevante — mientras la liquidez de venta sigue posicionada.", "El rebote sube de lado, sin prisa y sin lotes: nadie grande comprando, solo recompra goteada.", "El rebote parece un intermedio: tape lento, prints pequeños, y la oferta estructurada sigue montada justo arriba.", "Tres velas de alza y ni una huella institucional en el tape — volumen seco, delta irrelevante.", "El precio respira, el tape dormita: el rebote ocurre en el vacío, sin agresión compradora digna de nota.", "Rebote flaco tras un tramo gordo: los lotes que bajaron el mercado no aparecen del lado de la compra.", "El movimiento contrario llega sin convicción — negocios dispersos, tamaño diminuto, book vendedor intacto."],
      ...
      mistos: {
        ...
        tape: ["El tape alterna: agresión fuerte en una vela, ausencia en la siguiente; delta y precio cuentan historias distintas en piernas consecutivas.", "Una vela trae agresión pesada, la siguiente llega vacía; el delta apunta a un lado y el cierre va al otro — nada se confirma.", "El tape no construye narrativa: prints grandes aislados, sin continuidad, con precio y delta contradiciéndose en cada pierna.", "El tape cambia de opinión en cada vela: agresión compradora ahora, vendedora después, y el precio no valida ninguna.", "Volumen alto aquí, silencio allá; delta positivo en un print, negativo en el otro — la subasta no tiene dueño.", "Nada cuadra: la vela sube con delta vendedor, la siguiente cae con delta comprador, y ningún nivel se defiende dos veces.", "Señales en cortocircuito — cada evidencia apunta a un lado y la siguiente la desmiente.", "El mercado imprime ruido con volumen: movimientos que nacen fuertes y mueren en la vela siguiente.", "Lectura borrosa a propósito: agresiones aisladas, sin secuencia, sin defensa, sin narrativa."],
...
const emptyData = () => ({
  ...
  trialStart: Date.now(),
  beltIdx: 0, recent: [], lastBonusDay: null, regenAt: null, adViews: null,
  mistakes: [],
...
function beltFromCorrect(c) { return BELT_THRESHOLDS.reduce((acc, t, i) => (c >= t ? i : acc), 0); }
// faixa sobe só com taxa >= 60% nos últimos 60 cenários; congela, nunca rebaixa
function updateBelt(d) {
  const earned = beltFromCorrect(d.correct);
  const w = d.recent.length;
  const rate = w ? d.recent.filter(Boolean).length / w : 1;
  if (earned > d.beltIdx && (w < 30 || rate >= 0.6)) d.beltIdx = earned;
  return d;
}
// Master sem pontos regenera 10/h (até 100 acumulados)
function applyRegen(d, now) {
  if (d.tier !== "master") { d.regenAt = null; return d; }
  if (d.points >= SCENARIO_COST) { d.regenAt = null; return d; }
  if (!d.regenAt) { d.regenAt = now; return d; }
  const hrs = Math.floor((now - d.regenAt) / 3600000);
  if (hrs > 0) {
    d.points = Math.min(d.points + hrs * REGEN_PER_HOUR, REGEN_CAP);
    d.regenAt = d.points >= REGEN_CAP ? now : d.regenAt + hrs * 3600000;
  }
  return d;
}
function grantDailyBonus(d, today) {
  if (d.lastBonusDay !== today) { d.points += DAILY_BONUS; d.lastBonusDay = today; return true; }
  return false;
}
// anúncio recompensado (Parceiro do Dojo)
function adEligible(cfg, d, today) {
  if (!cfg || !cfg.enabled || !cfg.mediaUrl) return false;
  const aud = d.tier ? (d.tier === "base" ? "base" : null) : "free";
  if (!aud || !(cfg.audience || []).includes(aud)) return false;
  const n = d.adViews && d.adViews.d === today ? d.adViews.n : 0;
  return n < (cfg.dailyCap || 3);
}
function registerAdView(d, today) {
  const n = d.adViews && d.adViews.d === today ? d.adViews.n : 0;
  d.adViews = { d: today, n: n + 1 };
  return d;
}
...
export default function App() {
  ...
  function serveScenario(d) {
    const rd = applyRegen({ ...d }, Date.now());
    if (rd.points !== d.points || rd.regenAt !== d.regenAt) { setData(rd); persist(rd); }
    if (limitReached(rd)) { setPlanReason("trial"); setView("plan"); setScenario(null); return; }
    if (rd.points < SCENARIO_COST) { setPlanReason("points"); setView("plan"); setScenario(null); return; }
    const cat = pickCategory(rd.byCat, recentCats);
    setRecentCats((r) => [cat, ...r].slice(0, 2));
    const sc = GEN[cat]();
    sc.v = pickVariation(cat); // variação de tape sem repetição imediata no tema
    const nd = { ...rd, points: rd.points - SCENARIO_COST };
    if (nd.tier === "master" && nd.points < SCENARIO_COST && !nd.regenAt) nd.regenAt = Date.now();
  ...
  function respond(choice) {
    ...
    setData((prev) => {
      ...
      const next = {
        ...
        byCat: { ...prev.byCat },
        recent: [...(prev.recent || []), ok].slice(-60),
  ...
  function finishAd() {
    const pts = (adCfg && adCfg.rewardPoints) || 20;
    setData((prev) => {
      const nd = registerAdView({ ...prev, points: prev.points + pts }, todayStr());
      persist(nd);
      return nd;
    });
    setAdReward(pts);
  ...
  const beltIdx = data.beltIdx || 0;
  const beltEarned = beltFromCorrect(data.correct);
  const recentRate = data.recent && data.recent.length ? Math.round((data.recent.filter(Boolean).length / data.recent.length) * 100) : 100;
  const beltFrozen = beltEarned > beltIdx;
  const adOk = adEligible(adCfg, data, todayStr());
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: "Inter, system-ui, sans-serif", fontSize: 19, lineHeight: 1.55 }}>
      <div className="max-w-3xl mx-auto px-4 py-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4" aria-live="polite">
          ...
          <Chip label={U.points} value={data.points} highlight={data.points <= SCENARIO_COST * 10} />
        ...
        {beltFrozen && (
          <p style={{ color: C.orange, fontWeight: 700, marginTop: -6, marginBottom: 12, fontSize: 16 }}>
            {U.beltFrozen.replace("{r}", recentRate)}
          </p>
        )}
        ...
        {view === "plan" && (
          <main>
            <section style={{ background: C.surface, borderRadius: 18, border: "2px solid " + C.orange, padding: 24, textAlign: "center" }}>
              ...
              {planReason === "points" && data.tier === "master" && (
                <p style={{ color: C.orange, fontWeight: 800, marginBottom: 14 }}>{U.ptsRegen}</p>
              )}
              {planReason === "points" && adOk && (
                <button onClick={() => setAdOpen(true)}
                  style={{ ...btn, background: C.orange, color: "#231000", padding: "16px 28px", marginBottom: 12, display: "block", marginLeft: "auto", marginRight: "auto" }}>
                  🎬 {U.adWatch} ({(adCfg && adCfg.durationS) || 30}s) → +{(adCfg && adCfg.rewardPoints) || 20} {U.points}
                </button>
              )}
              {planReason === "points" && adReward > 0 && (
                <p style={{ color: C.buy, fontWeight: 800, marginBottom: 12 }}>✔ +{adReward} — {U.adDone}</p>
              )}
              {planReason === "points" && (
...
function AdModal({ cfg, U, onDone, onClose }) {
  const total = (cfg && cfg.durationS) || 30;
  const [left, setLeft] = useState(total);
  useEffect(() => {
    const t = setInterval(() => setLeft((v) => (v > 0 ? v - 1 : 0)), 1000);
    return () => clearInterval(t);
  }, []);
  useEffect(() => { if (left === 0) { const t = setTimeout(onDone, 400); return () => clearTimeout(t); } }, [left]);
  return (
    <div role="dialog" aria-label={U.adHead} style={{ position: "fixed", inset: 0, background: "rgba(6,7,24,0.92)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: C.surface, border: "2px solid " + C.orange, borderRadius: 18, maxWidth: 560, width: "100%", padding: 18 }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 10 }}>
          <span style={{ color: C.orange, fontWeight: 800, letterSpacing: "0.06em" }}>🎬 {U.adHead}{cfg && cfg.headline ? " · " + cfg.headline : ""}</span>
          <span style={{ color: C.text, fontWeight: 800, fontSize: 20 }}>{left}s</span>
        </div>
        {cfg && cfg.mediaType === "video" ? (
          <video src={cfg.mediaUrl} autoPlay muted playsInline style={{ width: "100%", borderRadius: 12, background: "#000" }} />
        ) : (
          <img src={cfg ? cfg.mediaUrl : ""} alt={U.adHead} style={{ width: "100%", borderRadius: 12 }} />
        )}
        <div style={{ background: C.card, height: 12, borderRadius: 8, marginTop: 12 }}>
          <div style={{ width: ((total - left) / total) * 100 + "%", height: "100%", background: C.orange, borderRadius: 8, transition: "width 1s linear" }} />
        </div>
        <p style={{ color: C.muted, fontSize: 15.5, marginTop: 10 }}>{U.adSkipNote}</p>
        {left === 0 ? (
          <p style={{ color: C.buy, fontWeight: 800, marginTop: 8 }}>✔ {U.adDone}</p>
        ) : (
          <button onClick={onClose} style={{ background: "none", border: "none", color: C.muted, fontSize: 15, marginTop: 8, cursor: "pointer", textDecoration: "underline" }}>×</button>
        )}
      </div>
    </div>
  );
